...
# ─────────────────────────────────────────────
log_step "0. Verifikasi Environment & CUDA"
...
# Cek dataset
# ─────────────────────────────────────────────
log_step "1. Cek Dataset COCO Subset"
if [ ! -f "$DATA_ROOT/annotations/instances_val2017_subset.json" ]; then
    log_warn "Dataset belum ada. Mengunduh dan menyiapkan subset 500 gambar..."
...
# Update _base_ di config eksperimen agar mengarah ke file yang benar
...
# ─────────────────────────────────────────────
# Kondisi A — FP16 (Mixed Precision / Strict)
# ─────────────────────────────────────────────
log_step "3. Kondisi A: FP16 (Original Co-DETR Benchmark)"

mkdir -p "$RESULTS_DIR/conditionA_fp16"
START_A=$(date +%s)

# Jalankan dengan train_with_monitor.py
# Tambahkan flag --strict-fp16 untuk memaksa perhatian (attention) di FP16 murni 
# agar mereproduksi overflow aktivasi secara andal
...
# ─────────────────────────────────────────────
# Kondisi B — FP32 Baseline
# ─────────────────────────────────────────────
log_step "4. Kondisi B: FP32 Baseline (Original Co-DETR Benchmark)"
...
# Ringkasan final
# ─────────────────────────────────────────────
log_step "✅ EKSPERIMEN BENCHMARK SELESAI"
echo ""
...
log_info "  └── REPORT.md"
echo ""
log_info "Anda dapat melakukan push dan pull hasil di laptop Anda."
